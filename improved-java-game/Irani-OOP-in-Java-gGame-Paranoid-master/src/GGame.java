...
public class GGame implements Runnable
{
	...
	public static GameElementAdapter[] getEntities() { //با همین متد به خاطر رفرنس بودن آرایه در جاوا میشه تغییرات را نیز روی آرایه اعمال کرد
		return Entities;
	}

	public static int getcEntities() {
		return cEntities;
	}

	public static void setcEntities(int cEntities) {
		GGame.cEntities = cEntities;
	}
	...
	// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Add Entities
	public int setNextEntity(GameElementAdapter element)
	{
	...
	public static void deleteEntityByEntityArray (int ID) { //ترتیب عناصر رو حفظ میکنه
		for (int x=ID; x<cEntities-1; x++) {
	...
	public static void replaceEntityByEntityArray (int a, int b) {
		GameElementAdapter Temp = Entities[a];
	...
	public LevelStatus Game_Loop()
	{
		...
		while (true)
		{
			...
			if (Levels.getLevel() == 10 && currentPlayer.getCurrentTime() == 60) {
				Levelstatus = LevelStatus.LevelWined;
			}

			System.out.println("cEntities= " + cEntities);
			System.out.println("Levels.getCTirBrick()= " + Levels.getCTirBrick());
			System.out.println("Levels.getCBeton()= " + Levels.getCBeton());
			System.out.println("getTotalFires()= " + getTotalFires());
			if (cEntities == Levels.getCBeton()+3 && Levels.getCTirBrick() == 0 && Levels.getCBeton()>0
					&& getTotalFires() == 0) {
	...
	public void run() {
		...
		{
			Levels.SetLevel(this , curLevel++); // curLevel++
